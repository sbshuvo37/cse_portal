...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_file') {
        ...
        if (!$isAssigned) {
            Helper::setFlash('danger', 'You are not assigned to this course + batch.');
            ...
            if ($res['success']) {
                ...
                $notifModel->createBulk(array_column($students, 'user_id'), 'Result File Uploaded', trim($_POST['file_title'] ?? 'Result File'), 'result');
                Helper::setFlash('success', 'Result file uploaded successfully.');
            } else {
...
// Fetch Admin uploaded final results
$db = Database::getInstance()->getConnection();
$adminFiles_stmt = $db->query("SELECT r.*, b.batch_name FROM result_files r JOIN batches b ON r.batch_id = b.batch_id WHERE r.course_id IS NULL ORDER BY r.created_at DESC");
$adminFinalResults = $adminFiles_stmt->fetchAll();
...
<div class="page-header">
    <div><h2>📈 Result Dashboard</h2><div class="page-subtitle">Enter students marks or manage files</div></div>
    <button class="btn btn-primary" onclick="openModal('fileUploadModal')">+ Upload Result File</button>
</div>

<!-- SECTION 1: MANUALLY ENTER MARKS (With beautiful border & separation) -->
<div class="card mb-3" style="border: 1.5px solid var(--primary-light); border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow);">
    <div class="card-header" style="background: var(--primary-pale); border-bottom: 1.5px solid var(--primary-light);">
        <span class="card-title" style="color: var(--primary-dark); font-weight: 800;">✍️ Manual Marks Entry Section</span>
    </div>
    <div class="card-body">
        <form method="GET" class="search-bar" id="filterForm">
            <!-- FIXED THE CASCADE SELECT JS BUG HERE -->
            <select name="batch_id" id="batchSelect" class="form-control" style="max-width:260px;" onchange="const cs = document.getElementById('courseSelect'); if(cs) cs.value=''; document.getElementById('filterForm').submit();">
        ...
        <table id="dataTable">
            ...
            <?php if (empty($students)): ?>
                <tr><td colspan="8" class="text-center">No students in this batch.</td></tr>
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-sm btn-accent" onclick="openMarkModal(<?= $s['student_id'] ?>, '<?= htmlspecialchars(addslashes($s['name'])) ?>', '<?= htmlspecialchars($s['roll']) ?>', <?= $existing['attendance'] ?? 0 ?>, <?= $existing['mid1'] ?? 0 ?>, <?= $existing['mid2'] ?? 0 ?>, <?= $existing['mid3'] ?? 0 ?>)">✏️ Marks</button>
                </td>
            </tr>
        ...
        <?php else: ?>
            <div class="empty-state"><p>Please select a Batch and Course above to enter student marks.</p></div>
        <?php endif; ?>
    </div>
</div>

<!-- SECTION 2: TEACHER FILES & ADMIN RESULTS -->
<div class="card mb-3" style="border: 1.5px solid var(--accent); border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow);">
    <div class="card-header" style="background: rgba(14,165,233,0.06); border-bottom: 1.5px solid var(--accent);">
        <span class="card-title" style="color: var(--accent-dark); font-weight: 800;">📄 Uploaded Result Sheets & Final Admin Results</span>
    </div>
    <div class="card-body">
        <h4 class="mb-2">📋 My Uploaded Course Result Files</h4>
        <?php if (empty($myResultFiles)): ?>
            <p class="text-muted" style="font-size:0.85rem; margin-bottom:18px;">No files uploaded yet.</p>
        <?php else: ?>
        <div style="display:flex; flex-direction:column; gap:10px; margin-bottom:24px;">
        <?php foreach ($myResultFiles as $f): ?>
        <div style="border: 1px solid var(--border); padding: 12px; border-radius: var(--radius); display:flex; align-items:center; justify-content:space-between; background: #fafbfc;">
            <div>
                <div style="font-weight:700;"><?= htmlspecialchars($f['title']) ?></div>
                <div style="font-size:0.78rem; color:var(--text-muted);"><?= htmlspecialchars($f['course_code'] ?? '') ?> — <?= htmlspecialchars($f['batch_name'] ?? '') ?></div>
            </div>
            <div style="display:flex; gap:6px;">
                <a href="<?= UPLOAD_URL . htmlspecialchars($f['file_path']) ?>" target="_blank" class="btn btn-sm btn-outline">View File</a>
                <form method="POST" style="margin:0;" onsubmit="return confirmAction('Delete this file?')">
        ...
        <!-- TEACHERS CAN VIEW THE ADMIN-UPLOADED BATCH FINAL RESULTS -->
        <h4 class="mb-2" style="border-top: 1px solid var(--border); padding-top:16px;">📈 Admin Uploaded Final Results</h4>
        <?php if (empty($adminFinalResults)): ?>
            <p class="text-muted" style="font-size:0.85rem;">No official final results published yet.</p>
        <?php else: ?>
        <div style="display:flex; flex-direction:column; gap:10px;">
        <?php foreach ($adminFinalResults as $f): ?>
        <div style="border: 1px solid var(--border); padding: 12px; border-radius: var(--radius); display:flex; align-items:center; justify-content:space-between; background: #f0fdf4; border-left: 4px solid var(--success);">
            <div>
                <div style="font-weight:700; color:var(--success);"><?= htmlspecialchars($f['title'] ?: 'Final Result') ?></div>
                <div style="font-size:0.78rem; color:var(--text-muted);">Batch: <?= htmlspecialchars($f['batch_name']) ?> · Published <?= Helper::timeAgo($f['created_at']) ?></div>
            </div>
            <a href="<?= UPLOAD_URL . htmlspecialchars($f['file_path']) ?>" target="_blank" class="btn btn-sm btn-success">View Final Result</a>
        </div>
        <?php endforeach; ?>
        </div>
        <?php endif; ?>
