...
$batchIds  = array_unique(array_column($courses, 'batch_id'));

$term = trim($_GET['q'] ?? '');
$results = ['courses'=>[], 'students'=>[], 'notices'=>[], 'resources'=>[]];

if ($term !== '') {
    $like = "%$term%";

    // Courses (only their assigned ones)
    ...
    // Students (only from their batches)
    $studentModel = new Student();
    foreach ($batchIds as $bid) {
        foreach ($studentModel->byBatch($bid) as $s) {
            if (stripos($s['name'], $term) !== false || stripos($s['roll'], $term) !== false) {
                $results['students'][] = $s;
            }
        }
    }

    // Notices (all visible ones)
    ...
    // Resources (their own)
    foreach ($resourceModel->byTeacher($teacher['teacher_id']) as $r) {
...
<h3 class="mb-2">📚 Your Courses (<?= count($results['courses']) ?>)</h3>
<div class="card mb-3"><div class="card-body" style="padding:0"><div class="table-wrapper"><table>
<thead><tr><th>Code</th><th>Title</th><th>Batch</th></tr></thead><tbody>
<?php foreach ($results['courses'] as $c): ?>
<tr><td><span class="badge badge-info"><?= htmlspecialchars($c['course_code']) ?></span></td><td><?= htmlspecialchars($c['course_title']) ?></td><td><?= htmlspecialchars($c['batch_name']) ?></td></tr>
<?php endforeach; ?>
...
<div class="card mb-3"><div class="card-body" style="padding:0"><div class="table-wrapper"><table>
<thead><tr><th>Name</th><th>Roll</th></tr></thead><tbody>
<?php foreach ($results['students'] as $s): ?>
<tr><td><?= htmlspecialchars($s['name']) ?></td><td><?= htmlspecialchars($s['roll']) ?></td></tr>
<?php endforeach; ?>
</tbody></table></div></div></div>
<?php endif; ?>
...
<h3 class="mb-2">📁 Your Resources (<?= count($results['resources']) ?>)</h3>
