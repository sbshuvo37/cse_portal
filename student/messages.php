...
// Allowed contacts: Admin + teachers of my batch's assigned courses
...
if ($student['batch_id']) {
    foreach ($courseModel->getCoursesForBatch($student['batch_id']) as $c) {
        $myTeachers[$c['teacher_id']] = $c['teacher_name'];
    }
}
// teacher_id here maps to teachers.teacher_id; we need user_id for messaging
$teacherModel = new Teacher();
$teacherContacts = [];
foreach ($myTeachers as $tid => $name) {
    $t = $teacherModel->findById($tid);
    if ($t) $teacherContacts[] = ['id'=>$t['user_id'], 'name'=>$t['name'], 'role'=>'teacher'];
}
...
<div class="page-header">
    <div><h2>💬 Messages</h2><div class="page-subtitle">Message your teachers or the admin</div></div>
